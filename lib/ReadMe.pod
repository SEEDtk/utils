...
This module contains code that is intended to be shared to other projects. It requires
no other modules to run.

=item utils

This module contains code that does not relate to the database and requires only
modules in C<RASTtk> to run.
...
When you add a module, you need to include it in the B<@modules> member of L<FIG_Config>
in order to get it to work in Eclipse. In Unix, you need to add the module name to the
modules list in C<seedtk-setup> and rerun the C<bootstrap> script.
...
in L<ExchangeFormat>. This directory can be overridden on the command line
for most scripts (see L<Shrub/new_for_script>).
...
You should be on the I<Main> tab. Type C<utils> in the I<Project> box and C<scripts/Config.pl> in the
