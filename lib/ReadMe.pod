...
To test a query, you must enter an L<object name list|ERDB/Object Name List> in the B<Path>
box, a L<filter clause|ERDB/Filter Clause> in the B<Filter> box, one or more parameter values
in the B<Parms> box, and a field name list in L<standard field name format|ERDB/Standard Field Name Format>
in the B<Fields> box. These all correspond to parameters of the method L<ERDB/GetAll>.

The B<Limit> box limits the number of rows returned. The B<Code Style> box causes PERL code for the
query to be displayed. A code style of C<GetAll> displays a single call to L<ERDB/GetAll>. A code style
of C<Get> displays a loop that uses the method L<ERDB/Get>. A code style of C<get_all> generates a command line
...
PERL or using a C<.pl> extension. Use

    source pull-all.sh

to pull down all the projects from GIT.

=head2 Starting a Windows Command Shell

To start a command shell under Windows, you also navigate to the project
directory. To establish the proper execution search paths, enter
...
to pull all the projects from GIT.
...
Installation files for external tools are kept in the C<packages> directory of the
main B<SEEDtk> project. Each tool is associated with a POD document that explains
